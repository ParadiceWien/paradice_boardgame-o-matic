...
function displayFilterValuesInResultDetails() {
      ...
      DISPLAY_ANSWERS_TO_QUESTIONS_IN_RESULT_DETAILS.questionsToBeDisplayed.forEach(
         (question) => {
            ...
            objCustomQuestion.arPositionValues.forEach((value, index) => {
               lookupTableEntry[value] = objCustomQuestion.arButtonAltTexts
                  ? objCustomQuestion.arButtonAltTexts[index]
                  : objCustomQuestion.arButtonLabels[index];
            });
            window.lookupTableForCustomQuestions[question.questionNr] =
               lookupTableEntry;
         }
   ...
   function addAnswersToDivContent(divContent, description) {
      ...
      DISPLAY_ANSWERS_TO_QUESTIONS_IN_RESULT_DETAILS.questionsToBeDisplayed.forEach(
         (question) => {
            const answerIndex =
               resultNr * intQuestions + (question.questionNr - 1);
            ...
            if (
               ...
               const personalPosition =
                  arPersonalPositions[question.questionNr - 1];
               let displayedValue;
               let elementClass;
               let tooltip;
      ...
      FILTERS.forEach((filter) => {
         ...
         if (filter.displayFilterValuesInResultDetails.bulletList) {
            ...
            presentFilterOptions.forEach((option) => {
               ...
               if (option.help) {
                  divContent += `<button class="bx bxs-help-circle icon-help icon-help-result-details-${
                     filter.internalName
...
function showHelpModalExplainingFilterOption(heading, body, examples) {
   ...
   if (helpModal) {
      ...
      helpModal.querySelector(".modal-body").innerHTML = `<p>${body}</p>`;
      if (examples !== "undefined") {
         helpModal.querySelector(".modal-body").innerHTML +=
            `<p class="examples-help-modal-filter-option">${PREFIX_EXAMPLES_HELP_MODAL_FILTER_OPTION} ${examples}</p>`;
      }
   } else {
   ...
   function createButtonResetAllFilters() {
      ...
      document
         ...
         .addEventListener("click", () => {
            ...
            document
               .querySelectorAll(
                  ".row-with-one-result[class*='hidden-by-filter']"
               )
               .forEach((result) => {
