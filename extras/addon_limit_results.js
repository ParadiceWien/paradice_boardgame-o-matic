...
function mow_addon_limit_results_create_buttons() {
   ...
   if (resultsHeadingContent) {
      ...
      let buttonContent_All = `<button type="button" class="Buttons_showPartiesAtEnd_all"><small>${TEXT_RESULTS_BUTTON_SHOW_ALL} (${intParties})</small></button>`;
      ...
      for (i = 0; i <= intQuestions - 1; i++) {
         ...
         element_resultsByThesisTable_col_row.className =
            "row showAlwaysIsTrue"; // "showAlwaysIsTrue" ist eine Pseudo-CSS-Klasse. -> nur für andere Addons als Warnung, z.B. "addon_results_textfilter.js"
      ...
      if (
         ...
         window.addEventListener("message", (event) => {
            if (event.data === "filter changed") {
               fnCalculate_Buttons(
                  0,
                  intPartiesCurrentlyShowing >= 5
                     ? intPartiesCurrentlyShowing
                     : 5
               );
            }
         });
...
function fnCalculate_Buttons(rowStart, rowEnd) {
   ...
   for (var i = 0; i < buttons_all.length; i++) {
      ...
      buttons_all[i].innerHTML =
         `<small>${TEXT_RESULTS_BUTTON_SHOW_ALL} (${intMaxPartiesToShow})</small>`;
   }
...
function fnShowOnlyIntPartiesAtEnd(rowStart, rowEnd) {
   ...
   for (i = 0; i < intTableLength; i++) {
      ...
      // Alle Zeilen, die außerhalb des Limits liegen -> ausblenden!
         ...
         resultsShortTable_rows[i].classList.add(
            "hidden-by-limit-results-addon"
         );
      }
   } // end: for-intParties
